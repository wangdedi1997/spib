...
python run_spib.py	-dt	# Time delay delta t in terms of # of minimal time resolution of the trajectory data
			...
			-SaveTrainingProgress	# Whether save training progress

Sample command:
    python run_spib.py -dt 50 -d 1 -encoder_type Nonlinear -bs 512 -threshold 0.01 -patience 2 -refinements 8 -lr 0.001 -b 0.01 -seed 0 -label examples/Four_Well_beta3_gamma4_init_label10.npy -traj examples/Four_Well_beta3_gamma4_traj_data.npy
...
def test_model():
    ...
    if '-SaveTrainingProgress' in sys.argv:
        SaveTrainingProgress = bool(sys.argv[sys.argv.index('-SaveTrainingProgress') + 1])
    else:
